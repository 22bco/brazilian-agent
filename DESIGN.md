...
|---|---|---|
| **Python + FastAPI** | Async webhook, clean LLM SDK integration, legible repo | (none) |
...
| **Leads to Google Sheets via Apps Script** | Reviewers watch leads appear live, no service-account dance | Manual one-time deploy; public web-app URL guarded by a shared secret |
...
- **Slugs** (`torres-del-paine`) are mapped to readable names with correct accents.
...
generically** (lead-row `n_days` not equal to booked nights), flags that trip's
duration as not-confident, and the prompt instructs the agent to **not state a
fixed number** and describe it by itinerary or confirm with a human.
...
- **Kapso** (`app/channels/kapso.py` + `/webhook`) is the primary one. Recommended
  by the brief, no ban risk, matches the sandbox tester-number flow. Async fast-ack.
- **Baileys** (`baileys/`, a Node sidecar) is the creative bet: self-hosted, with
